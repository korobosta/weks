<?php 
	...
	if($_POST['id'] == ""){
		$check_repetition=$conn->query("SELECT * from fee WHERE course='$course' AND semester='$semester' AND school_year='$school_year'");
	...
	if ($sql=mysqli_query($conn, "INSERT into fee (course, semester, amount,school_year) 
		VALUES ( '$course', '$semester', '$amount','$school_year')")){
	...
	setTimeout(function(){	window.location.href='rms.php?page=configure_fee';  }, 2000);</script>";
		...
		$id=$_POST['id'];
		$check_repetition=$conn->query("SELECT id from fee WHERE course='$course' AND semester='$semester' AND school_year='$school_year' AND id != '$id'");
	if(mysqli_num_rows($check_repetition)>0){
		echo "<b style='color:red'>The Course, semester and school year should be unique for each entry</b> ";
		...
		$sql = "UPDATE fee SET course='$course',semester='$semester',amount='$amount',school_year='$school_year' WHERE id='$id'";
		mysqli_query($conn, "INSERT into history_log (transaction,user_id,date_added) 
