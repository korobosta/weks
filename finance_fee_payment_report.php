...
?>
	...
	<tr>
		...
		<p style="padding:5px; font-weight: bold; color:green" >Fee Payments for students </p>
		<p style="padding:10px; font-weight: bold; color:teal"></p>	
		</td>
  ...
  <table id="students" class="table table-bordered" >
    ...
    $sql=  mysqli_query($conn, "SELECT * FROM payments left join grade on payments.semester=grade.grade_id JOIN school_year ON payments.school_year=school_year.SY_ID JOIN  students ON payments.student=students.STUDENT_ID join user on students.USER=user.USER_ID ");
    if(!$sql){
      echo $conn->error;
    }
